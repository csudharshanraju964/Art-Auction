...
var swiper = new Swiper(".swiper-container", {
  ...
  // add event on active slide while sliding
  swiper.on('slideChange', function () {
    let img_containers = document.getElementsByClassName('swiper-slide');
    for(let i=0; i<img_containers.length; i++) {
      img_containers[i].removeEventListener("click",showImageAndNavbar)
    }
    let activeSliderImage=document.getElementsByClassName("swiper-slide-active")[0]
    
    activeSliderImage.addEventListener("click",showImageAndNavbar)
    activeSliderImage.addEventListener("click",showActiveSlideImageinPop)
  });
  
  
  // get elements and add event listener on elements
  let activeSliderImage=document.getElementsByClassName("swiper-slide-active")[0]
  activeSliderImage.addEventListener("click",showImageAndNavbar)
  activeSliderImage.addEventListener("click",showActiveSlideImageinPop)
  
  let toggle=true
  let Image_navbar_container=document.getElementsByClassName("auction-info-container")[0]
  let Image_container=document.getElementsByClassName("image-container")[0]
  let InfoSidebar=document.getElementsByClassName("info-card")[0]
  let infoIcon=document.getElementById('info-image')
  Image_container.addEventListener("click",hideImageAndNavbar)
  
  function showImageAndNavbar(){
    Image_navbar_container.style.display="block"
  }
  function hideImageAndNavbar(){
    Image_navbar_container.style.display="none"
  ...
  // toggle auction infosidebar
  function toggleInfocard(){
      if(toggle){
        InfoSidebar.style.display="block"
        infoIcon.style.filter="brightness(0) invert(1) sepia(1) saturate(10000%) hue-rotate(45deg)";
        toggle=false
      }else{
        InfoSidebar.style.display="none"
        infoIcon.style.filter="none";
  ...
  // get the active slide image to show in pop
  function showActiveSlideImageinPop(){
    let activeSlideImage=document.querySelector(".swiper-slide-active img")
    let popupImage=document.querySelector("#popUpImage")
    popupImage.src=activeSlideImage.src
